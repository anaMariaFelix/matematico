package com.anamariafelix.matematico.file.export.impl;
...
public class XlsxExporter implements FileExporter {
    ...
    public Resource exportFile(List<PersonDTO> people) throws Exception {
        try (Workbook workbook = new XSSFWorkbook()){
            Sheet sheet = workbook.createSheet("People");//cria uma aba

            Row headerRow = sheet.createRow(0);//cria o headres na linha 0
            String[] headers = {"ID", "First Name", "Last Name", "Address", "Gender", "Enabled"};//cria o cabeçario da planilha

            for (int i = 0; i < headers.length; i++) {//intera sobre os headres
